...
import MiniLineChart from '@/components/ui/charts/MiniLineChart.vue';
import { ref } from 'vue';
...
<template>
    <SectionCard class="w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 justify-between p-0">
        <template v-for="(item, index) of statsData" :key="index">
            <div class="sm:flex-1 flex xl:flex-row flex-col items-center gap-6 sm:odd:border-r md:last:border-none md:even:border-r p-6 md:last:mr-0 border-surface-200 dark:border-surface-800">
                <div class="lg:flex-1 w-full">
                    ...
                    <div class="flex items-center gap-1 mt-2">
                        ...
                        <Tag :severity="item.status" :value="item.change" />
                    </div>
                </div>
                <MiniLineChart className="!max-h-16 min-w-[60px] h-full lg:flex-1 w-full [&_canvas]:!w-full" :data="item.data" :bgColor="item?.bgColor" :borderColor="item?.borderColor" />
            </div>
        </template>
    </SectionCard>
</template>
