...
import ImageCropperStencil from './ImageCropperStencil.stage.vue';
...
const upload = async (file) => {
    ...
    try {
        ...
        toast.add({
            severity: 'error',
            summary: 'Error uploading image file',
            life: 3000
        });
    }
...
<template>
    <div class="relative flex flex-col gap-4 w-full min-h-[300px]">
        <Cropper ref="reference" :src="props.image" :stencil-component="ImageCropperStencil" @ready="onReady" />
        ...
        <div v-else class="sticky bottom-0 flex justify-center">
            <Button :loading="uploading" :disabled="uploading" icon="pi pi-check" label="Save" @click="onSave" />
        </div>
    </div>
</template>
