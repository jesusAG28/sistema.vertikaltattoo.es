...
import ImageCropper from './ImageCropper.stage.vue';
...
const validateLocalFile = (file) => {
    if (file.size > props.maxSize) {
        errors.value.push(`Max file size: ${FileService.size(props.maxSize)}`); // TODO: add new translation or find existing (validation) translation
    }
};
            ...
            <div class="flex flex-col flex-wrap justify-center items-center gap-4">
                <div class="relative w-64 h-64 shrink-0 flex justify-center items-center">
                    <img
                        ...
                        class="w-full h-full object-scale-down rounded-full bg-[var(--image-dropzone-preview-bg)] hover:bg-[var(--image-dropzone-preview-hover-bg)]"
                    />
                    <div v-if="unlink && props.image && !croppedImage" class="flex justify-center items-center">
                        <Button text rounded @click="restoreImage" icon="pi pi-replay" title="Restore initial image" />
                    </div>
                    <div v-if="!unlink || croppedImage" class="absolute top-0 right-0">
                        <Button severity="danger" text rounded icon="pi pi-times" :title="`${croppedImage ? 'Clear' : 'Unlink'} image`" @click="clearImage" />
                    </div>
                </div>
                <div class="flex flex-col justify-center items-center gap-2 px-[var(--p-form-field-padding-x)] py-[var(--p-form-field-padding-y)]">
                    <Button outlined @click="openLocalFileChooser" icon="pi pi-plus" label="Choose" />
                    <span>or drop image here.</span>
                </div>
            </div>
        </div>
        <Dialog header="Dialog" v-model:visible="displayCropper" :breakpoints="{ '991px': '50vw' }" :style="{ maxWidth: '75vw' }" :modal="true">
            <ImageCropper :image="FileService.createObjectURL(choosenImage)" @uploaded="uploaded" />
            <template #footer />
        </Dialog>
    </div>
</template>
