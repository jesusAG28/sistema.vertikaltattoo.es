    ...
    zodResolver(
        z
            .object({
                password: z
                    .string()
                    .nonempty({ message: t('validation.required', { attribute: t('reset_password.form.password') }) })
                    .max(255, { message: t('validation.max.string', { attribute: t('reset_password.form.password') }) }),
                password_confirmation: z
                    .string()
                    .nonempty({ message: t('validation.required', { attribute: t('reset_password.form.password_confirmation') }) })
                    .max(255, { message: t('validation.max.string', { attribute: t('reset_password.form.password_confirmation') }) })
            })
            .refine((data) => data.password === data.password_confirmation, {
                message: t('validation.confirmed', { attribute: t('reset_password.form.password') }),
...
const submit = async ({ valid, values }) => {
    if (!route.query.email || !route.query.token || !valid) {
        return;
    }
    ...
    try {
        await AuthService.resetPassword(route.query.email, route.query.token, values.password, values.password_confirmation);

        router.push('/');
    } catch (AxiosError) {
        if (AxiosError.response?.status === 401) {
            const error = {
                summary: t('reset_password.error.summary'),
                detail: t('reset_password.error.credentials.detail')
            };
            ...
            <div class="relative mt-28 max-w-[36rem] mx-auto">
                ...
                >
                    <div class="pt-8 pb-8">
                        ...
                        <h1 class="text-4xl lg:text-5xl font-semibold text-surface-950 dark:text-surface-0 text-center">{{ $t('reset_password.title') }}</h1>
                        <p class="text-center lg:text-xl text-surface-500 dark:text-white/64 mt-6 max-w-sm mx-auto">{{ $t('reset_password.heading') }}</p>
                    </div>
                    ...
                    <div class="flex flex-col gap-2">
                        <label for="reset_password.password" class="font-medium text-surface-500 dark:text-white/64 first-letter:uppercase">{{ $t('reset_password.form.password') }}</label>
                        <Password inputId="reset_password.password" name="password" toggleMask :feedback="false" fluid :invalid="!!backendErrors.password || $form.password?.invalid" @focusin="delete backendErrors.password" />
                    ...
                    <div class="flex flex-col gap-2">
                        <label for="reset_password.password_confirmation" class="font-medium text-surface-500 dark:text-white/64 first-letter:uppercase">{{ $t('reset_password.form.password_confirmation') }}</label>
                        <Password
                            inputId="reset_password.password_confirmation"
                    ...
                    <div class="flex items-center">
                        <Button type="submit" :loading="submitting" :icon="$form.valid && !hasAdditionalErrors ? 'pi pi-check' : null" :label="$t('reset_password.form.submit')" :disabled="!$form.valid || hasAdditionalErrors || submitting" fluid />
                    </div>
                </Form>
            </div>
        </div>
