...
const resolver = ref(
    zodResolver(
        z.object({
            email: z
                .string()
                .nonempty({ message: t('validation.required', { attribute: t('login.form.email') }) })
                .email({ message: t('validation.email', { attribute: t('login.form.email') }) })
                .max(255, { message: t('validation.max.string', { attribute: t('login.form.email') }) }),
            password: z
                .string()
                .nonempty({ message: t('validation.required', { attribute: t('login.form.password') }) })
                .max(255, { message: t('validation.max.string', { attribute: t('login.form.password') }) }),
            remember: z.boolean()
        })
    )
);
...
const submit = async ({ valid, values }) => {
    ...
    try {
        ...
        if (AxiosError.response?.status === 401) {
            const error = {
                summary: t('login.error.summary'),
                detail: t('login.error.credentials.detail')
            };
...
<template>
    <section class="animate-fadein animate-duration-300 animate-ease-in relative lg:pb-34 py-32">
        <div class="landing-container mx-auto relative z-10 px-12">
            <div class="relative max-w-[46rem] mx-auto">
                <div
                ...
                >
                    <div class="pt-8 pb-8">
                        <div class="flex items-center justify-center p-12">
                            <!-- <AuthLogoWidget /> -->
                            <img class="logo-image w-32" :src="'/layout/images/logo-fetlpa.webp'" alt="FETLPA" />
                        </div>
                        <h1 class="text-4xl lg:text-6xl font-semibold text-surface-950 dark:text-surface-0 text-center">{{ $t('login.title') }}</h1>
                        <!-- <p class="text-center lg:text-xl text-surface-500 dark:text-white/64 mt-6 max-w-sm mx-auto">{{ $t('login.heading') }}</p> -->
                    </div>
                    ...
                    <div class="flex flex-col gap-2">
                        <label for="login.email" class="font-medium text-surface-500 dark:text-white/64 first-letter:uppercase">{{ $t('login.form.email') }}</label>
                        <InputText id="login.email" name="email" fluid :invalid="!!backendErrors.email || $form.email?.invalid" @focusin="delete backendErrors.email" autocomplete="off" />
                    ...
                    <div class="flex flex-col gap-2">
                        <label for="login.password" class="font-medium text-surface-500 dark:text-white/64 first-letter:uppercase">{{ $t('login.form.password') }}</label>
                        <Password inputId="login.password" name="password" toggleMask :feedback="false" fluid :invalid="!!backendErrors.password || $form.password?.invalid" @focusin="delete backendErrors.password" />
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-2">
                            <Checkbox inputId="login.remember" name="remember" binary @change="toggleRemember" />
                            <label for="login.remember" class="text-surface-500 dark:text-white/64 first-letter:uppercase">{{ $t('login.remember_me') }}</label>
                        </div>
                        <!-- <router-link to="/forgot-password" class="font-semibold text-primary">{{ $t('login.forgot_password') }}</router-link> -->
                    </div>
                    <div class="flex items-center">
                        <Button type="submit" :loading="submitting" :icon="$form.valid && !hasAdditionalErrors ? 'pi pi-check' : null" :label="$t('login.form.submit')" :disabled="!$form.valid || hasAdditionalErrors || submitting" fluid />
                    </div>
                </Form>
            </div>
        </div>
