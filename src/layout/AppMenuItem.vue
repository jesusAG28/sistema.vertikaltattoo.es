...
watch(
    () => route.path,
    (newPath) => {
        if (!(isSlim.value || isCompact.value || isHorizontal.value) && props.item.to && ((props.item.to !== '/' && newPath.includes(props.item.to)) || props.item.to === newPath)) {
...
<template>
    <li ref="menuItemRef" :class="{ 'layout-root-menuitem': root, 'active-menuitem': isStatic ? !root && isActiveMenu : isActiveMenu }">
        <div v-if="root && item.visible !== false" class="layout-menuitem-root-text">{{ item.translatable ? $t(item.translatable) : item.label }}</div>
        <a
            ...
            v-tooltip.hover="isCompact && root && !isActiveMenu ? (item.translatable ? $t(item.translatable) : item.label) : null"
        >
            <i :class="item.icon" class="layout-menuitem-icon" />
            <span class="layout-menuitem-text label-small text-inherit">{{ item.translatable ? $t(item.translatable) : item.label }}</span>
